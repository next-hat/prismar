...
use prismar_schema::{Field, FieldAttribute, FieldType, GeneratorValue, Model, Schema, find_generator};

const PRISMAR_PROVIDER: &str = "prismar-cli";

pub fn run_generate(schema_path: &Path, output_override: Option<PathBuf>) -> Result<()> {
  ...
  fs::write(output_dir.join("schema.rs"), schema_rs)
    .with_context(|| format!("failed to write {}", output_dir.join("schema.rs").display()))?;
  ...
  for model in &schema.models {
    ...
    fs::write(output_dir.join(format!("{module_name}.rs")), rendered).with_context(|| {
      format!(
        "failed to write {}",
  ...
  fs::write(output_dir.join("mod.rs"), mod_rs)
    .with_context(|| format!("failed to write {}", output_dir.join("mod.rs").display()))?;

  println!("Generated {} model module(s) in {}", schema.models.len(), output_dir.display());
  Ok(())
}
...
fn resolve_output_dir(
  ...
  if let Some(generator) = find_generator(schema, PRISMAR_PROVIDER)
    .or_else(|| schema.generators.iter().find(|generator| generator.name == "prismar"))
  {
  ...
  Ok(schema_path.parent().unwrap_or_else(|| Path::new(".")).join("generated"))
}
  ...
  for model in &schema.models {
    for relation in relation_metadata(model) {
      if relation.fields.len() == 1 {
        ...
        let parent_table = schema.models.iter().find(|item| item.name == relation.parent_model)
          .map(table_name)
...
fn render_model_file(schema: &Schema, model: &Model) -> Result<String> {
  ...
  let updatable = persisted
    ...
    .collect::<Vec<_>>();
  if pk.is_empty() {
    return Err(anyhow!("model {} must define at least one @id or @@id field", model.name));
  }
  ...
  out.push_str(format!("#[derive({})]\n", derive_list(db_derives, config_list(generator, "db_derives"))).as_str());
  out.push_str(format!("#[diesel(table_name = super::schema::{table_name})]\n").as_str());
  ...
  out.push_str(format!("#[derive({})]\n", derive_list(default_partial_derives(), config_list(generator, "partial_derives"))).as_str());
  out.push_str(format!("#[diesel(table_name = super::schema::{table_name})]\n").as_str());
  ...
  for field in &persisted {
    ...
    out.push_str(format!("  pub {}: Option<{}>,\n", field.name, rust_type(field, true)).as_str());
  }
  out.push_str("}\n\n");

  if updatable.is_empty() {
    out.push_str(format!("#[derive({})]\n", derive_list(default_empty_update_derives(), config_list(generator, "update_derives"))).as_str());
    out.push_str("#[allow(dead_code)]\n");
    out.push_str(format!("pub struct {update_name};\n\n").as_str());
  } else {
    out.push_str(format!("#[derive({})]\n", derive_list(default_update_derives(), config_list(generator, "update_derives"))).as_str());
    out.push_str(format!("#[diesel(table_name = super::schema::{table_name})]\n").as_str());
    out.push_str("#[allow(dead_code)]\n");
    out.push_str(format!("pub struct {update_name} {{\n").as_str());
...
fn active_generator(schema: &Schema) -> Option<&prismar_schema::Generator> {
  find_generator(schema, PRISMAR_PROVIDER)
    .or_else(|| schema.generators.iter().find(|generator| generator.name == "prismar"))
}

fn persisted_fields(model: &Model) -> Vec<&Field> {
  model.fields.iter().filter(|field| !matches!(field.r#type, FieldType::Relation(_))).collect()
}

fn single_primary_key_field<'a>(model: &'a Model) -> Option<&'a Field> {
...
fn primary_keys(model: &Model) -> Vec<String> {
  let mut keys = model.fields.iter().filter(|field| field.attributes.iter().any(|attr| matches!(attr, FieldAttribute::Id))).map(|field| field.name.clone()).collect::<Vec<_>>();
  if !keys.is_empty() {
    return keys;
  }
...
fn table_name(model: &Model) -> String {
  model.mapped_name.clone().unwrap_or_else(|| to_snake_case(&model.name))
}
...
fn config_list(generator: Option<&prismar_schema::Generator>, key: &str) -> Vec<String> {
  generator.and_then(|generator| generator.config.get(key)).and_then(|value| match value {
...
fn config_bool(generator: Option<&prismar_schema::Generator>, key: &str, default: bool) -> bool {
  generator.and_then(|generator| generator.config.get(key)).and_then(|value| match value {
...
fn has_updated_at(field: &Field) -> bool {
  field.attributes.iter().any(|attr| matches!(attr, FieldAttribute::UpdatedAt))
}
...
fn parse_name_list(raw: &str) -> Vec<String> {
  raw.split(',').map(str::trim).filter(|item| !item.is_empty()).map(|item| item.trim_matches('"').to_owned()).collect()
}
...
fn render_model_traits(schema: &Schema, model: &Model, out: &mut String) -> Result<()> {
  let model_name = &model.name;
  let db_name = format!("{}Db", model_name);
  let create_name = format!("{}Create", model_name);
  let update_name = format!("{}Update", model_name);
  ...
  out.push('\n');
  out.push_str(format!("impl prismar::PrismaCreateData for {create_name} {{\n").as_str());
  out.push_str(format!("  type Model = {db_name};\n\n").as_str());
  render_model_create_normalizer(schema, model, out)?;
  out.push_str("}\n\n");

  out.push_str(format!("impl prismar::PrismaUpdateData for {update_name} {{\n").as_str());
  out.push_str(format!("  type Model = {db_name};\n").as_str());
  out.push_str("}\n\n");
  ...
  out.push_str(format!("impl prismar::PrismaModel for {db_name} {{\n").as_str());
  ...
  out.push_str(format!("  fn primary_key_field() -> &'static str {{ \"{}\" }}\n\n", pk_field.name).as_str());
  ...
  out.push_str(format!("        {}\n", parse_id_from_field_filter(pk_field).trim_end()).as_str());
  out.push_str("      }\n");
  out.push_str("      _ => Err(prismar::RuntimeError::InvalidFilter(format!(\"expected equality predicate on {}\", expected))),\n");
  out.push_str("    }\n  }\n\n");

  out.push_str(format!("  async fn create(client: &prismar::PrismaClient, data: Self::Create) -> Result<usize, prismar::RuntimeError> {{\n").as_str());
  render_backend_dispatch(schema, out, |backend| format!(
    "      client.{backend}(move |conn| {{ diesel::insert_into(super::schema::{table}::table).values(&data).execute(conn) }}).await\n"
  ));
  out.push_str("  }\n\n");

  out.push_str(format!("  async fn find_many(client: &prismar::PrismaClient, filter: Option<prismar::ModelFilter>) -> Result<Vec<Self>, prismar::RuntimeError> {{\n").as_str());
  ...
  out.push_str(format!("  async fn find_by_id(client: &prismar::PrismaClient, id: &Self::Id) -> Result<Option<Self>, prismar::RuntimeError> {{\n").as_str());
  ...
  out.push_str(format!("  async fn update_by_id(client: &prismar::PrismaClient, id: &Self::Id, data: Self::Update) -> Result<usize, prismar::RuntimeError> {{\n").as_str());
  ...
  out.push_str(format!("  async fn delete_by_id(client: &prismar::PrismaClient, id: &Self::Id) -> Result<usize, prismar::RuntimeError> {{\n").as_str());
  out.push_str("    let id = id.clone();\n");
  render_backend_dispatch(schema, out, |backend| format!(
